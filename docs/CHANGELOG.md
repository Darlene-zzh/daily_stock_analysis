...
- [修复] Agent 分析路径进度条不再冻死在 58%：US 股等走 Agent 模式时，`pipeline._analyze_with_agent` 发出「58% 切换 Agent 分析链路」后，整个多步工具调用循环（外加可选委员会的约 12 次 LLM 调用）几分钟内零进度上报，进度条卡在 58%、任务持续显示「分析中」，看起来像卡死（实际仍在跑）。现 `AgentExecutor.run` 与 `AgentOrchestrator.run`（多 agent 模式生产实际用的是后者）均新增可选 `progress_callback`，透传到既有 agent loop / `_execute_pipeline`（runner 每步已发 thinking/tool_start/generating 事件），`_analyze_with_agent` 用新纯函数 `_agent_event_progress` 把这些事件映射成单调递增的 59→92 进度并 emit，agent 跑完补 94；`AnalysisService` 在调用委员会前补一个 96% 「投资委员会评议中…」tick。普通（非 Agent）路径不受影响。新增 9 个单测（含签名回归守卫：两条 `run` 入口都必须接受 `progress_callback`，防止只改一条导致多 agent 路径 TypeError 全挂）；全后端套件通过零回归。
- [修复] 仓位流水汇总「最差止损」不再把盈利离场显示成红色亏损：当止损价高于成本（移动止损保护浮盈，如 MSFT 成本 $396.81 / 止损 $415.25 → 离场仍 +8.67），其 worst-case P&L 实为正数，但前端 `PositionFlowTimeline` 硬编码红色、双渲染器（`notification.py` / `history_service.py`）裸打数字，看起来像亏损。现三处统一按金额正负决定显示：≥0 时绿色加「+」前缀（仍盈利），<0 时红色保留原生「-」。后端 `_compute_position_outcome_summary` 计算逻辑不变（数值本就带符号正确）。纯显示修复，已入库报告刷新页面即生效，无需重新分析。新增后端渲染符号测试 3 个（含双渲染器字节一致）+ 前端正负色彩测试 2 个。
- [修复] 分析完成后任务不再卡在「分析中 XX%」：activeTasks 的状态转换此前完全依赖 SSE `task_*` 事件，但 SSE 无事件重放——全新分析耗时数分钟，期间连接若断开重连，漏掉的 `task_completed` 不会补发，任务被永久滞留在「分析中」列表（SSE `onTaskCompleted` 里 2 秒自动移除的逻辑因事件没到而从不触发），而 30 秒轮询只刷新历史、从不与真实任务状态对账。新增纯函数 `reconcileActiveTasks`（依赖注入、可单测）作为兜底：每 15 秒及标签页重新可见时，对每个 pending/processing 任务调用 `GET /analysis/status/{id}` 拿后端真值，驱动与 SSE 相同的 completed/failed/progress 回调，孤儿任务在下一个 tick 清除。`useDashboardLifecycle` 把 SSE 完成/失败处理抽成共享 handler 复用。新增前端单测 6 个；纯前端改动，后端无变更。
- [修复] 持仓操作计划 narrative 不再谎报止损/止盈与成本的关系：`sanitizer_v2` 新增 Check #11，用 FactBundle 里的 `portfolio.avg_cost` 与每条 item 的 `trigger_price` 实际关系，确定性纠正 narrative 中"低于/高于…成本"的比较词。线上 MSFT 复现：3×ATR 移动止损 $415.25 正确地高于成本 $396.81（保护浮盈），但 LLM 把 narrative 写成"低于平均成本"——价位对，只有散文撒谎。纠正只改比较副词（低于↔高于），保留其余文案；trigger 在成本 ±0.5% 内（"接近成本"）或无 `portfolio.avg_cost` fact 时不动；与既有 ghost-gate 同为确定性防御层、不依赖 LLM。新增 6 个单元测试，全后端套件 2598 通过零回归。仅影响新分析，已入库的旧报告需重新分析（非缓存命中）才会刷新。
- [修复] 24h 同股缓存命中时投委会面板不再静默消失：`AnalysisService._build_analysis_response` 末尾新增 `committee` / `risk_assessment` 顶层 lift，缓存路径（`_lookup_recent_cache_response` → `_build_analysis_response`）此前只把这两份 payload 写到 `report.dashboard.{committee, risk_assessment}` 而不写顶层，但前端 `ReportSummary` 从 `report` 顶层解构 `{committee, riskAssessment}`，结果用户在 24h 内重新分析同一只股时（如 CRDO 22:29 后第二天再点），SSE `task_completed` 事件携带的 result 顶层 committee 缺失 → 投委会会议纪要面板不渲染，必须刷新走 `GET /history/{id}`（已有 lift）才能看到。lift 对 fresh 分析路径是 no-op（committee hook 在该 builder 之后才写顶层），对缓存路径恰好接住。新增 4 个 TDD 测试（缓存命中 lift committee / lift risk_assessment / 无 committee 时不污染顶层 / lift 是 copy 不是 move 故 dashboard.committee 保留供 Markdown renderer 用）。全后端套件 2592 通过零回归。
- [改进] FactBundle 候选硬化（ghost-price gate）：`src/analysis/candidate_rules.py` 在规则跑完后新增 `_apply_ghost_gate`，把 (a) 价格 ≤ 0、(b) 距现价 |%| > 50（`GHOST_DISTANCE_THRESHOLD_PCT`）、(c) 源 fact 本身腐败（`technical.atr_14 ≤ 0` 或 `≥ current`、`quant.qlib_rank > 1.0`）的候选统一改标 `tier='filtered'`，sanitizer Check #4 随即把指向它们的操作项 drop。`sanitizer_v2._candidate_is_ghost` 作为防御深度独立校验（覆盖手工构造 bundle 路径）。覆盖此前的 7 类已知腐败信号面：NVDA 风格 pre-split 阻力 ghost、20 日 swing low/high ghost、Trading 212 decimal-shift 导致的 99999 / 0.01 成本锚 ghost、qlib 模型分位 > 1.0 数据 glitch、A 股筹码均价 0/负、技术支撑位 0/负、ATR 0/负/超过现价、MA10/MA20 0/负。15 条原 `xfail(strict=True)` 测试翻转为常规断言，11 条 "pin today's behavior" 测试的 tier 期望同步翻转。`tests/test_candidate_price_sanity.py` 36/36 通过、全后端套件 2588 通过零回归。
- [修复] 持仓操作计划不再泄漏陈旧/策略不一致的条目：`_try_inject_action_plan_items` 主路径结尾此前是 `if items: core_out["action_plan_items"] = items`——当 LLM 推荐 `wait_and_see` 或 sanitizer 把候选全部清空时（`items` 为空），不会覆盖 `core_conclusion.action_plan_items`，导致主分析阶段预先写入的未打标、可能与策略矛盾的原始项（如 `wait_and_see` 下残留一个 `buy`、`provenance=None`）泄漏到报告。现改为 inject 步骤**无条件**作为 `action_plan_items` 的权威来源（即使为空也覆盖），合法的 `wait_and_see` 正确渲染为无操作计划。新增 1 个复现测试。
- [修复] LLM 不可用时持仓操作计划不再为空：`_try_inject_action_plan_items` 此前在 `generate_text` 返回空/None（额度耗尽、provider 宕机、fallback 链全挂）时直接 return，跳过了已有的候选合成兜底——报告会出现空的"持仓操作计划"。现改为：LLM 无可用输出时从 `dashboard.fact_bundle` 的候选规则化合成一份代码接地的方案（无需 LLM），并把 `generate_text` 调用包一层防御以容忍抛异常。新增可复用 helper `_synthesize_action_plan_from_bundle` 统一三处合成逻辑（原 LLM 空、upstream sanitizer 空、LLM 不可用），消除重复。新增 2 个单元测试（LLM 返回 None / 抛异常均应合成出方案）。
- [新功能] FactBundle 候选规则补齐至 spec 全量 23 条：`src/analysis/candidate_rules.py` 新增 `support_breakdown`（支撑跌破止损 / stop_loss，基于 `technical.support`，support < 现价时发射）、`ma20_pullback`（MA20 回踩入场 / entry，基于 `technical.ma20`，ma20 ≤ 现价时发射）、`breakout_retest`（突破回踩入场 / entry，基于 `technical.swing_high_20d`，前期波段高点被上破后变支撑、swing_high < 现价时发射）。三条规则均复用既有 fact、与对偶规则（support_test / ma20_breakdown / prev_swing_high）共享 fact 但方向不同，tier=primary，无新增 fact 依赖。新增 6 个单元测试（发射 + 跳过边界各 3），离线套件 2570 通过。
- [改进] Web 端可访问性：`EvidenceRef` 引用 pill 与 `CommitteeMinutesPanel` 缺席原因把原生 `title` hover 提示改为 `aria-label`（原生 title 对键盘/读屏不可达，被 UI 治理守卫禁止）；`agentChatStore` 的 `getInitialSessionId` 在模块初始化读取 `localStorage` 时增加方法可用性检测与 try/catch，隐私模式/SSR/沙箱环境下不再崩溃。
- [测试] 修复 dsa-web 测试基础设施：`src/setupTests.ts` 新增内存版 `localStorage`/`sessionStorage`（jsdom 在本配置下仅提供无方法的空对象，导致任何在导入期读取存储的模块崩溃）；修正 `HomePage` 空状态测试断言（旧 "开始分析" EmptyState 已被组合持仓热力图 treemap 取代）与 `stockPoolStore` 默认 `reportType`（`detailed` → `full`，与 store/`api/analysis.ts` 两处默认源一致）。本轮 web 测试套件从 41 失败恢复到 0 失败（519 通过）。
- [新功能] FactBundle Phase 3 渲染层落地（双 renderer + Wikipedia 风格脚注）：`src/notification.py` 与 `src/services/history_service.py` 的 `_render_action_plan_items` 在 `fact_bundle` 存在时为 4 个依据行（触发/技术面/基本面/量化）按 evidence_refs 类型前缀附加 ¹²³ 上标（unicode 数字），并对 `provenance == "synthesized"` 的条目展示 🤖 *代码兜底* 徽章。新增 `_render_evidence_footnotes(refs, fact_bundle)` 辅助函数，每只股票的通知/历史 Markdown 报告在持仓操作计划之后追加完整脚注块（按首次出现顺序去重、unknown fact_id 显示 "(引用未在 FactBundle 中找到)"）。新增 `tests/test_renderer_parity_phase3.py` 强制两份 renderer 输出字节相等，防止 [[repo-dual-renderers]] 漂移；新增 NVDA fixture 与 23 个单元/集成测试。`fact_bundle` 缺失或 `evidence_refs` 为空时输出与改造前字节相同——老报告完全兼容。
- [新功能] FactBundle Phase 5 落地（前端集成 + 报告页全面升级）：首页个股报告的决策区改用证据接地组件——`ReportSummary` 顶部挂载 `PriceMapCard`（现价 + 候选支撑/阻力/止损/目标横向价位图，内嵌"刷新价格"按钮调 `stocksApi.getQuote`，坐标轴钉在原始现价避免刷新抖动，并按规则去重/限量短标签）；`ActionPlanTable` 重写为渲染 `EvidenceExpansion` 证据折叠面板 + `narrative` 文案 + `provenance`（🤖 代码兜底）/`tier` 徽章 + 内联 `EvidenceRef` 引用 pill；新增 `StrategyHeroCard`（hero 策略 + 备选 + citation pill + provenance）替换旧 `StrategyThesis` 与 `StrategySelector`；新增 `PositionFlowTimeline`（触发时间线 + 持仓结果摘要网格）替换旧 `PositionOutcomeSummary`；删除 3 个旧组件并更新 `index.ts`。老报告（无 `fact_bundle` / 无 `evidence_refs`）回退到既有展示，零破坏。
- [修复] FactBundle Phase 5 hotfix：`api/v1/schemas/history.py` 的 `DashboardSection` / `ActionPlanItemSchema` / `StrategyChoiceSchema` 补齐 Phase 1+2 字段（`fact_bundle` / `evidence_refs` / `provenance` / `tier` / `candidate_id` / 结构化 `strategy_thesis`）；此前 Pydantic `response_model` 默认 `extra='ignore'` 把这些字段从 `/api/v1/history/{id}` 响应里静默剥离，导致前端组件拿不到数据库里已存的证据数据。
- [修复] FactBundle Phase 5 hotfix：`sanitizer_v2.sanitize_with_candidates` 新增 `portfolio_held` 参数（默认 True 向后兼容），为 False 时清除每个存活 item 上的 `shares` / `pct_of_position` / `pct_of_equity`，避免 LLM 对不存在的持仓臆造"持仓 5%"类字段。
- [修复] FactBundle Phase 5 hotfix：`src/analyzer.py` 的 `_try_inject_zh_translations` 不再把已是中文的源字段镜像进 `*_zh` 槽（源已中文时留空，渲染器回退到原字段），并在 `src/notification.py` 与 `src/services/history_service.py` 双渲染器加 `zh != src` 防御，消除双语情报区中文重复打印。
- [新功能] FactBundle Phase 4 落地（前端基建 + 实时报价接口）：新增 `GET /api/v1/stocks/{code}/quote` 实时行情接口（返回 `StockQuote`）；前端新增 `stocksApi.getQuote` helper、`useFactBundle` hook（`getFact(id)` / `byType(type)` / `byPrefix(prefix)`）以及 4 个 atomic 组件（`EvidenceRef` 引用 pill / `EvidenceExpansion` 证据折叠面板 / `RefreshPriceButton` / `PriceMapCard`）。本期组件就绪但未挂载，对用户可见行为零变更——为 Phase 5 集成铺垫。
- [新功能] FactBundle Phase 2 落地（LLM 证据接地）：`build_strategy_classify_prompt` 接受 `fact_bundle` 参数后，会在策略分类提示词中追加三段新内容——`format_facts_table`（按 technical/quant/committee/intel/portfolio/flow/chip 分组的事实清单）、`format_candidates_menu`（候选触发价位 Markdown 表，带 ID/方向/距现价/规则/适用策略/层级）、`OUTPUT_CONTRACT_ZH`（要求 LLM 必须从候选池选 `candidate_id`、附 ≥2 个 `evidence_refs`、写 `narrative`、标 `tier`/`provenance`）。新增 `sanitizer_v2.sanitize_with_candidates`（9 条候选锚定校验：candidate_id 存在性、trigger_price 强制对齐、applicable_strategies、filtered tier、direction vs current_price、evidence_refs 自动补齐到 ≥2、discipline_anchor 至多 1 条、同 ID 去重、priority 重排）；新增 `synthesizer.synthesize_from_candidates` 在 LLM/sanitizer 清零时按策略从 candidates 池规则化生成兜底 items（每条带 `provenance: "synthesized"`）。`StockAnalysisPipeline` 将 `_attach_fact_bundle` 调用移到 `_try_inject_action_plan_items` 之前，`GeminiAnalyzer._sanitize_action_plan_items` / `_try_inject_action_plan_items` 自动检测 dashboard.fact_bundle 并启用 v2 路径；无 bundle 时回退到 Phase 1 legacy 成本基础 sanitizer。本期对老报告兼容——所有新字段（`candidate_id`/`evidence_refs`/`narrative`/`tier`/`provenance`）均为 optional。33 个新单元测试 + 1 个 NVDA fixture E2E。
- [新功能] FactBundle 基础设施落地（Phase 1）：新增 `src/analysis/` 包，包含 `FactRecord`/`FactBundle`/`CandidateLevel` 类型、8 个 per-domain extractor（technical/quant/committee/intel/portfolio/flow/chip + ATR/swing 推导）、23 条候选触发价位规则（resistance_touch / ma20_breakdown / ma10_pullback / support_test / atr_2x/3x / R-multiple 2R/3R / resistance_plus_atr / psychological_round / cost ±5/12/20% / cost−10% / prev_swing_high/low / fib_extension 1272/1618 / qlib_top_decile_buy / chip_avg_cost），并由 `facts_builder.build_fact_bundle(...)` 统一编排。`StockAnalysisPipeline.process_single_stock` 在分析成功后调用 `_attach_fact_bundle` 将 `dashboard.fact_bundle` 作为新字段挂载（59 个新 pytest 单元 + 1 个 NVDA fixture E2E）。本期对用户可见行为零变更——为后续证据接地决策管线（Phase 2-5）打基础；任何加载/计算失败都被 try/except 吞掉，dashboard 不变更。
- [修复] 实时分析与任务轮询响应现在把 `committee` / `risk_assessment` 从 `dashboard` 提升到 `AnalysisReport` 顶层（与 history GET 端点一致）。此前 `api/v1/endpoints/analysis.py` 的 `_build_analysis_report` 与 `get_task_status` 完成分支只透传 `dashboard`，未提升这两个 opt-in 字段；而前端 `ReportSummary` 从 `report.committee` / `report.riskAssessment` 顶层解构，导致投委会纪要与结构化风险面板在新分析完成时不渲染，必须重新走历史详情才出现（投委会纪要本就在初始入库后才延迟回填）。新增模块级 `_lift_optin_payloads` helper 统一两处提升逻辑，缺失/空字典时为 None（可选面板不渲染），向后兼容。新增 2 个契约测试。
...
- [修复] 持仓 snapshot 顶层 currency 不再硬编码 CNY：单账户时自动取该账户 base_currency；多账户时可通过 `PORTFOLIO_REPORT_CURRENCY` 覆盖（默认仍为 CNY）。响应新增 `fx_fallback_used` 标记，前端在 stale 且发生 1:1 fallback 时显示明确提示，避免 GBP/USD 金额被误标为 CNY。
...
- [chore] react-doctor 批量收口 Tailwind 简写：`w-N h-N` 折叠为 `size-N`（106 处）、`px-N py-N` 折叠为 `p-N`（6 处），共 30 个文件 119 处替换；regex 用 prefix + value 双 backref 保证只折叠对称对（`md:` / `hover:` / `dark:` 等变体保持原状）。运行时语义零变化。
- [chore] react-doctor 批量收口 barrel imports：把 `from '../common'`、`from '../../hooks'`、`from '../stores'`、`from '../components/dashboard'`、`from '../components/settings'`、`from '../components/StockAutocomplete'`、`from '../components/history'`、`from '../components/report'` 等 8 个 barrel 入口替换为直接 import（29 个文件，覆盖 39 处 barrel 引用、共 81 行直接 import）。tree-shaking 边界更清晰，build 后 modules 3194 → 3192。注意：`useStockPoolStore` 文件名为 `stockPoolStore.ts`，`Shell` 实际在 `components/layout/` 跨目录重导，规则中按 diagnostic 建议路径做基名匹配避免假设错误。
- [改进] react-doctor 第三批低风险清理：（1）`apps/dsa-web/src/utils/format.ts` 把 4 处函数内 `new Intl.DateTimeFormat(...)` 提到模块顶层（`DATETIME_ZH` / `DATE_ZH` / `DATE_ISO_SHANGHAI`），格式化器复用一次性实例；（2）6 个 heading 字重从 `font-bold` / `font-extrabold` 改为 `font-semibold`（`ReportOverview` 股票名 h2、`NotFoundPage` h1、`ChatPage` 头部 h1、`LoginPage` "DAILY STOCK" 品牌 h2 + Analysis Engine h3 + 表单 h1），符合 react-doctor `design-no-bold-heading` 关于 ≥700 字重在大尺寸下挤压字形 counter 的建议；登录页品牌头视觉变化最显著（800 → 600）。
- [改进] `apps/dsa-web/src/components/common/Drawer.tsx` backdrop 由 `<div onClick>` 升级为 `<button type="button" aria-label="关闭抽屉">`，键盘 Tab 可达 + Enter/Space 关闭，document-level Escape 监听保持不变。`apps/dsa-web/src/components/common/ConfirmDialog.tsx` 加 useEffect Escape 监听 + 外层 `<div role="dialog" aria-modal="true">` 使用 target-check 模式（仅 backdrop 自身点击触发 cancel，dialog 卡片点击不再 stopPropagation），同时拆掉了内层 onClick={stopPropagation}。覆盖原先 6 条 click-events / no-static-element-interactions 警告中的 5 条；剩余 1 条 ConfirmDialog 外层 click-events 是 modal 经典模式（Escape 是主要键盘关闭路径）lint 无法识别的合理保留。
- [chore] react-doctor Medium+ 重构 `apps/dsa-web/src/components/settings/LLMChannelEditor.tsx`：把 1803 行单文件拆为 `LLMChannelEditor/` feature folder（types.ts / utils.ts / ChannelRow.tsx / LLMChannelEditor.tsx / index.ts + 兄弟 `__tests__/`），SettingsPage 改用直接路径 import；同步把 ChannelRow 内部再拆 `ChannelTestPanel` / `ChannelDiscoveryPanel` / `ChannelCapabilityPanel` 三个 sub-panel，函数体从 403 行降到 ~180 行，消掉 `no-giant-component` 警告。设计 + 实施 plan 见 `docs/superpowers/specs/2026-05-19-llmchanneleditor-decomposition-design.md` 和 `docs/superpowers/plans/2026-05-19-llmchanneleditor-decomposition.md`。
