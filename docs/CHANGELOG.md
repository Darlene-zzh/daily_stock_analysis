...
- [修复] Agent 分析路径进度条不再冻死在 58%：US 股等走 Agent 模式时，`pipeline._analyze_with_agent` 发出「58% 切换 Agent 分析链路」后，整个多步工具调用循环（外加可选委员会的约 12 次 LLM 调用）几分钟内零进度上报，进度条卡在 58%、任务持续显示「分析中」，看起来像卡死（实际仍在跑）。现 `AgentExecutor.run` 新增可选 `progress_callback` 透传到既有 agent loop（runner 每步已发 thinking/tool_start/generating 事件），`_analyze_with_agent` 用新纯函数 `_agent_event_progress` 把这些事件映射成单调递增的 59→92 进度并 emit，agent 跑完补 94；`AnalysisService` 在调用委员会前补一个 96% 「投资委员会评议中…」tick。普通（非 Agent）路径不受影响。新增 8 个进度映射单测；全后端套件 2609 通过零回归。
