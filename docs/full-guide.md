...
Web 端提供 **4 套主题风格 × 浅色/暗色** 两种模式：

| 风格 | 说明 |
|------|------|
| 高级（默认） | 墨蓝 + 钢蓝 + 香槟金点缀、衬线标题，沉稳专业 |
| 可爱 | 草莓奶昔粉，圆体字、贴纸感徽章、🍡 空状态，柔和甜美 |
| 简约 | 纯黑白灰、大留白，克制清晰 |
| 经典 | 青色科技玻璃拟态（旧版观感保留） |

- **切换入口**：侧边栏底部（移动端为顶栏）的「🎨 主题风格」下拉，或「设置 → 系统 → 外观」的预览卡，点选即时生效。
- **明暗模式**：与风格相互独立，可单独切换，默认跟随系统设置。
- **记忆方式**：选择保存在浏览器本地（`localStorage` 键 `dsa-theme-family`），换设备/浏览器不同步。
- **涨跌与盈亏配色**：沿用红涨绿跌（A 股惯例），各风格仅调整色相以协调；持仓热力图盈利绿/亏损红的色相也随主题变化。
- 桌面端（Electron）加载同一 Web 产物，主题能力一并生效，无需单独配置。
...
- 4 master 是否并行 fan-out 由 `INVESTMENT_COMMITTEE_MASTERS_PARALLEL`（默认 `false`）控制；付费层（OpenAI / Anthropic）可设 `true` 把 master 阶段耗时压到串行的约 1/4；免费层（Gemini / Cerebras / Groq）共享 RPM bucket，并行容易触发 429，**不要开**。
