...
def test_apply_portfolio_match_none_leaves_result_attribute_unset():
    """When the pipeline has no portfolio context, the helper must NOT set
    ``result.portfolio_match`` so the renderer falls back to the default
    two-row position-advice table.
    """
    pipeline = _build_pipeline()  # portfolio_match defaults to None
    assert pipeline.portfolio_match is None
    result = _make_result()
    assert not hasattr(result, "portfolio_match"), (
        "AnalysisResult defines no portfolio_match field — the helper is "
        "the only writer; this precondition guards the test."
    )
    _apply_portfolio_match(result, pipeline)
...
def test_renderer_emits_default_two_row_table_when_portfolio_match_unset(mock_get_config):
    ...
    service = NotificationService()
    result = _make_result()
    # portfolio_match deliberately left unset — _apply_portfolio_match
    # with pipeline.portfolio_match=None must NOT touch the attribute.
    assert not hasattr(result, "portfolio_match")
