...
def _render_structured_risk(
    ...
    opt-in.  Mirror of :func:`src.services.history_service._render_structured_risk`;
...
def _render_committee_minutes(
    ...
    Returns ``[]`` when ``committee`` is missing / empty so the renderer
    stays graceful for opt-out runs.  Section order (spec §10):

    1. Status banner (only when status != 'ok')
    2. PM verdict + score + rationale + dissents (suppressed when 'failed')
    3. Risk strip (severity + red flags + suggested position %)
    4. Debate timeline (compact Round-N bullets)
    5. Lens grid: persona display name + verdict + score + headline +
       'absent' badge when status='failed'

    The persona display strings come from the single-source-of-truth
    ``PERSONA_DISPLAY`` mapping; never duplicate strings between modules.
    """
    if not isinstance(committee, dict) or not committee:
        return []

    # Import lazily to avoid making notification.py depend on the agent
    # subtree at import time.
    try:
    ...
    # 2) PM verdict (skip the verdict card when failed — spec §10)
        ...
        for m in masters:
            ...
            if m_status != "ok":
                if lang == "en":
                    ...
                    badge = " _(缺席)_"
            # First-mention bilingual hint for zh mode
            zh_subtitle = ""
    ...
    # Budget footnote — keeps cost visible
...
class NotificationService(
            ...
            for result in sorted_results:
                ...
                if intel:
                    ...
                    def _zh_for(idx, zh_list):
                        """Return zh translation for list item idx or '' on fallback."""
    ...
    def generate_single_stock_report(self, result: AnalysisResult) -> str:
        ...
        # Sprint 1A: append Investment Committee Minutes when present.
        # The committee dict lives on result.dashboard["committee"] so the
        # existing dashboard-walking pipeline can surface it uniformly.
        ...
        if committee_data:
            try:
                ...
                logger.warning("[committee] render failed in single-stock report: %s", exc)

        # Sprint 4: standalone structured Risk Assessment.  Lives at
        # ``result.dashboard["risk_assessment"]`` and is INDEPENDENT of the
        # committee path — renders when the API caller opts in regardless
        # of committee state.
        ...
        if risk_assessment_data:
            try:
                ...
                logger.warning("[risk_assessment] render failed in single-stock report: %s", exc)
