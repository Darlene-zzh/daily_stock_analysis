...
"""
    ...
    def _try_inject_action_plan_items(
        ...
        """Post-process: ask the LLM specifically for action_plan_items.

        Called when the main analysis pass didn't fill action_plan_items (mini models
        routinely ignore extended-schema fields, see [[repo-llm-mini-models-schema]]).
        Unlike the main analysis prompt — which the LLM treats as one big task list and
        drops the field at the end — a small focused prompt with ONLY this responsibility
        gets a much higher fill rate. Fails silently; caller wraps in try/except so
        a sub-call hiccup never breaks the main pipeline.

        Cost-basis aware: the portfolio_context_block carries avg_cost, so this LLM call
        can produce a take_profit that respects what the user actually paid for the share.
        """
        if not result or not result.dashboard:
            return
        ...
        if isinstance(existing, list) and existing:
            return  # Main LLM already cooperated.

        # Distill the dashboard down to the minimum the LLM needs to reason — full
        # dashboard would blow the prompt budget and dilute the focus.
        ...
        import json as _json
        compact_json = _json.dumps(compact_input, ensure_ascii=False, indent=2, default=str)

        prompt = f"""你是为现有持仓用户生成结构化操作计划的助手。下面是用户的实际持仓信息（含成本价）和该股票当前的多维分析结果。请基于这些信息输出 2-4 条 action_plan_items。
...
## 要求
1. **必须基于用户实际成本价**（最关键约束）：
   - 持仓者的 `take_profit` 触发价必须**高于成本价**（至少 +1%），否则就是亏损不是止盈——这种情形请改为 "break-even" 退场触发或干脆不出 take_profit 项。
   - 持仓者的 `stop_loss` 触发价应当 **≤ 成本价 × 1.02**，否则它就不是「止损」（你不在亏损中），而是「在阻力/支撑位的防守性减仓」——这种情形请用 `direction = "sell"` 并在 trigger_condition 中说明是防守性降仓；不要用 "stop_loss" 标签。
   - 简单口诀：take_profit 必在成本上方，stop_loss 必在成本下方或接近成本；介于二者之间用 sell。
2. 综合上面的技术摘要、风险警报、利好催化、情绪综述给出依据；不要复述纯均线描述。
3. 触发价必须与 time_sensitivity 时间窗口内可能触发的水平一致。
4. 输出 2-4 条，按 priority 升序（1=最优先）。每条必须包含全部 11 个字段：
   - trigger_price (数值)
   - trigger_condition (含价位/量能/时间，1 句)
   - direction (buy / sell / stop_loss / take_profit)
   - shares (整数或小数股数；持仓者参考实际持股数量做拆分；未持有时按权益 5%-10% 给初始仓)
   - pct_of_position (持有时占当前持仓 %，未持有时 null)
   - pct_of_equity (占账户总权益 %)
   - technical_basis (引用具体指标数值)
   - fundamental_basis (引用具体新闻/财报点)
   - quant_signal (量比/换手率/筹码/资金流向；如缺失就填空字符串 "")
   - invalidation_rule (1 句，含具体价位)
   - priority (1-3)
...
"""
        ...
        raw = self.generate_text(prompt, max_tokens=2048, temperature=0.3)
        if not raw:
            return
        ...
        items = parsed.get("action_plan_items") if isinstance(parsed, dict) else None
        if not isinstance(items, list) or not items:
            return

        # Cost-basis sanitization: mini models routinely mislabel chart-derived levels —
        # they'll put direction="take_profit" on a price BELOW cost (a loss masquerading
        # as profit) or direction="stop_loss" on a price ABOVE cost (a defensive trim
        # masquerading as loss prevention). Fix both, since the user reads these labels
        # literally and acts on them.
        from src.services.portfolio_context_service import _parse_portfolio_facts
        avg_cost = _parse_portfolio_facts(portfolio_context_block).get("avg_cost")

        # Defensive normalize: keep only required-shape items and trim to 4.
        normalized: list = []
        for it in items[:4]:
            ...
            if trig is None or not direction:
                continue
            # Drop take_profit items at or below cost basis — they're not profitable exits.
            if (
                direction == "take_profit"
                and avg_cost is not None
                and isinstance(trig, (int, float))
                and trig <= avg_cost * 1.005  # within 0.5% of cost = not a real profit
            ):
                logger.info(
                    "[action_plan] dropping take_profit @ %s ≤ avg_cost %s for %s",
                    trig, avg_cost, code,
                )
                continue
            # Reclassify "stop_loss" items priced above cost as defensive sells. A real
            # stop_loss should be at or below cost basis (where the position becomes a
            # loss). If the chart's stop_loss is in profit territory, it's actually a
            # trailing-stop / profit-defense decision, not loss prevention.
            if (
                direction == "stop_loss"
                and avg_cost is not None
                and isinstance(trig, (int, float))
                and trig > avg_cost * 1.02  # at least 2% above cost = clearly profitable
            ):
                logger.info(
                    "[action_plan] reclassifying stop_loss @ %s > avg_cost %s as 'sell' for %s",
                    trig, avg_cost, code,
                )
                it = dict(it)
                it["direction"] = "sell"
        ...
        # Renumber priorities contiguously so the UI doesn't show gaps (e.g. 1, 3 with
        # no 2 when a middle item was filtered). Sort by emitted priority first to
        # preserve LLM's intended ordering.
        normalized.sort(key=lambda x: (x.get("priority") if isinstance(x.get("priority"), (int, float)) else 99))
        ...
        result.dashboard["core_conclusion"]["action_plan_items"] = normalized
