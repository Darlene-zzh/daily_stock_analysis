...
```json
{
    ...
    "dashboard": {
        "core_conclusion": {
            ...
            "_comment_action_plan": "以下字段仅在持仓上下文存在时输出；其他场景省略",
            "action_plan_items": [
                {
                    "trigger_price": 421.0,
                    ...
                    "pct_of_equity": 3.5,
                    "technical_basis": "RSI=74 超买，MACD 动能减弱，MA20 附近压力明显",
                    "fundamental_basis": "近期利空消息尚未完全 price-in",
                    "quant_signal": "量比 1.8 放量，主力净流出，筹码集中度下降",
                    "invalidation_rule": "放量收盘站稳 $428 以上则计划作废",
                    "priority": 1
                }
            ]
        },
...
"""
        ...
        if detect_market(stock_code) == "us":
            # 将 JSON schema 模板里的占位符替换成真实的 _zh 字段定义，
            # 让 LLM 的输出样例直接包含 _zh 字段，提高遵从率。
            _zh_inline = (
                '"sentiment_summary_zh": "sentiment_summary 的简体中文翻译",\n'
                '            "earnings_outlook_zh": "earnings_outlook 的简体中文翻译",\n'
                '            "risk_alerts_zh": ["risk_alerts[0] 的中文翻译", "risk_alerts[1] 的中文翻译"],\n'
                '            "positive_catalysts_zh": ["positive_catalysts[0] 的中文翻译"],'
...
"""
    ...
    def _try_inject_zh_translations(self, result: "AnalysisResult", code: str) -> None:
        """Post-process: translate intelligence fields to Chinese for US/zh reports.
        ...
        if detect_market(code) != "us":
            return
        if normalize_report_language(getattr(result, 'report_language', 'zh')) != "zh":
            return
        # Skip if _zh fields already present and non-empty
        ...
        fields_json = _json.dumps(to_translate, ensure_ascii=False, indent=2)
        prompt = (
            "请将以下美股分析字段翻译成简体中文。要求：\n"
            "1. 忠实原意，不增减内容\n"
            "2. 列表字段逐条翻译，顺序和条数不变\n"
            "3. 仅输出合法 JSON，键名加 _zh 后缀\n"
            "   (如 risk_alerts_zh, positive_catalysts_zh, latest_news_zh, "
            "sentiment_summary_zh, earnings_outlook_zh)\n"
            "4. 不输出任何其他内容\n\n"
            f"输入：\n{fields_json}\n\n输出（仅 JSON）："
        )

        raw = self.generate_text(prompt, max_tokens=1024, temperature=0.3)
        if not raw:
            return
        ...
        # Only inject recognized _zh keys to avoid polluting the intelligence dict
        valid_keys = {
            'risk_alerts_zh', 'positive_catalysts_zh', 'latest_news_zh',
            'sentiment_summary_zh', 'earnings_outlook_zh',
        }
    ...
    def _try_inject_action_plan_items(
        ...
        # position_outcome_summary, because Gemini 2.5 Flash routinely emits
        # strategy-inconsistent items (wait_and_see + stop_loss, stepped_profit_taking
        # with no take_profit ladder) and hallucinates outcome numbers.
        upstream_strategy = core.get("recommended_strategy")
        upstream_items = core.get("action_plan_items")
        if (
            isinstance(upstream_strategy, str) and upstream_strategy
            ...
            if isinstance(core.get("strategy_choices"), list):
                core["strategy_choices"] = self._sanitize_strategy_choices(
                    core["strategy_choices"], recommended_strategy=upstream_strategy
                )
        ...
        if isinstance(parsed.get("strategy_choices"), list):
            core_out["strategy_choices"] = self._sanitize_strategy_choices(
                parsed["strategy_choices"],
        ...
        # action_plan_items it just produced. Compute and recompute are now the
    ...
    _STRATEGY_MAX_ITEMS = {
        ...
        # wait_and_see = no actionable triggers by definition. We hard-cap at 0
        # so the post-process drops anything the LLM emits (it routinely emits
        # a stop_loss even when wait_and_see is the recommended strategy).
        "wait_and_see": 0,
    }
    ...
    def _compute_position_outcome_summary(
        ...
        OR when items is empty / contains no actionable triggers — a summary with
        worst=0 / best=0 / R:R=N/A would just be visual noise next to a wait_and_see
        plan, so the renderer is meant to hide the whole card in that case.
        """
        ...
        for it in items:
            ...
            try:
                ...
                continue
            # Resolve shares: explicit shares > 0 wins; otherwise derive from
            # pct_of_position × holding_shares. LLMs (esp. Gemini 2.5 Flash)
            # frequently emit shares=0 alongside pct_of_position=100 — drop the
            # item entirely if neither yields a positive share count, so the
            # outcome math doesn't pretend an action exists when it doesn't.
            shares_f: Optional[float] = None
            try:
                if shares is not None:
                    shares_f = float(shares)
            except (TypeError, ValueError):
                shares_f = None
            if (shares_f is None or shares_f <= 0) and pct is not None:
                try:
                    pct_f = float(pct)
                    if pct_f > 0:
                        shares_f = holding_shares * (pct_f / 100.0)
                except (TypeError, ValueError):
                    shares_f = None
            if shares_f is None or shares_f <= 0:
                continue
            # Cap derived shares to the actual holding so an LLM emitting 150%
            # doesn't fabricate a position larger than reality.
            shares_f = min(shares_f, holding_shares)
            actioned += shares_f
        ...
        # If every item was filtered out (e.g. all shares=0 with no pct, or all
        # directions forbidden), suppress the summary entirely instead of showing
        # a zero-loss / zero-gain card alongside an empty action table.
        if actioned <= 0 and worst_loss == 0 and best_gain == 0:
            return None
    ...
    def _recompute_position_outcome_summary(
        self,
        core_out: Dict[str, Any],
        sanitized_items: list,
        portfolio_context_block: Optional[str],
    ) -> None:
        """Replace any LLM-emitted ``position_outcome_summary`` with one computed
        from the sanitized action_plan_items + real portfolio facts.

        If portfolio context is missing or items are empty, ``position_outcome_summary``
        is removed entirely — better to show nothing than to surface a hallucinated
        £-figure that doesn't match the (now empty/wait-and-see) plan.
        """
        ...
        if outcome:
            ...
            core_out.pop("position_outcome_summary", None)

    _STRATEGY_LABEL_ZH_MAP = {
        "long_term_hold": "长线持有",
        "swing_trade": "短线波段",
    ...
    _STRATEGY_IDS_ORDER = ["long_term_hold", "swing_trade", "stepped_profit_taking", "wait_and_see"]

    def _sanitize_strategy_choices(
        self,
        raw_choices: list,
        recommended_strategy: Optional[str] = None,
    ) -> list:
        """Clean LLM-emitted strategy_choices.

        - Drop entries with no usable identifier AND no display text
        - Backfill label_zh + emoji from id lookup when missing
        - If LLM omitted id, attempt to recover by matching against the 4 canonical
          ids in positional order (only when the array has exactly 4 entries —
          the most common shape).
        - Ensure recommended_strategy's choice is marked applicable=True
        - Deduplicate by id (keep first occurrence)
        """
        if not isinstance(raw_choices, list):
            return []

        # Step 1: positional id recovery when LLM gave 4 entries without ids
        if len(raw_choices) == 4 and all(
            isinstance(c, dict) and not c.get("id") for c in raw_choices
        ):
            for i, c in enumerate(raw_choices):
                c["id"] = self._STRATEGY_IDS_ORDER[i]
        ...
        for c in raw_choices:
            if not isinstance(c, dict):
                continue
            sid = c.get("id")
            label = c.get("label_zh")
            reason = c.get("inapplicable_reason")
            fit = c.get("fit_condition")
            # Drop if no id AND no label AND no inapplicable_reason AND no fit_condition
            if not sid and not label and not reason and not fit:
                continue
            # Deduplicate by id (keep first; LLM sometimes repeats)
            if sid and sid in seen_ids:
                continue
            if sid:
                seen_ids.add(sid)
            # Backfill label_zh + emoji from canonical map when missing
            if sid and not label:
                c["label_zh"] = self._STRATEGY_LABEL_ZH_MAP.get(sid, sid)
            if sid and not c.get("emoji"):
                c["emoji"] = self._STRATEGY_EMOJI_MAP.get(sid, "📌")
            # Force recommended strategy's choice to applicable=True
            if sid and sid == recommended_strategy:
                c["applicable"] = True
                c.pop("inapplicable_reason", None)
            cleaned.append(c)
    ...
    def _sanitize_action_plan_items(
        ...
        # stepped_profit_taking: must have a 3-step take_profit ladder. Gemini 2.5
        # Flash routinely recommends the strategy in core_conclusion but omits the
        # actual ladder rows from action_plan_items (it puts the prose into
        # strategy_thesis instead). Without a concrete ladder the user sees "AI
        # 推荐分批止盈" but no specific trigger prices, which is exactly the
        # screenshot the user flagged. Synthesize a defensive baseline anchored on
        # cost basis when missing.
        if strategy == "stepped_profit_taking" and avg_cost is not None:
            existing_tps = sum(1 for it in normalized if it.get("direction") == "take_profit")
            if existing_tps == 0:
                tp1 = round(avg_cost * 1.05, 2)   # First lock: +5% from cost
                tp2 = round(avg_cost * 1.12, 2)   # Mid lock: +12%
                tp3 = round(avg_cost * 1.20, 2)   # Stretch lock: +20%
                normalized.append({
                    "trigger_price": tp1,
                    "trigger_condition": f"成本价上方 +5% 触发首档止盈",
                    "direction": "take_profit",
                    "shares": 0,
                    "pct_of_position": 30.0,
                    "technical_basis": "首档止盈：覆盖手续费与短期波动后锁定基础利润",
                    "fundamental_basis": "落袋为安第一档",
                    "quant_signal": None,
                    "invalidation_rule": "若价格回踩成本价以下则取消并降级为持有",
                    "priority": 50,
                })
                normalized.append({
                    "trigger_price": tp2,
                    "trigger_condition": f"成本价上方 +12% 触发中档止盈",
                    "direction": "take_profit",
                    "shares": 0,
                    "pct_of_position": 40.0,
                    "technical_basis": "中档止盈：均线多头排列下的合理止盈区",
                    "fundamental_basis": "落袋为安第二档（主仓位）",
                    "quant_signal": None,
                    "invalidation_rule": "若 MA 排列转空且跌破首档止盈位则提前减仓",
                    "priority": 51,
                })
                normalized.append({
                    "trigger_price": tp3,
                    "trigger_condition": f"成本价上方 +20% 触发尾档止盈（拉伸目标）",
                    "direction": "take_profit",
                    "shares": 0,
                    "pct_of_position": 30.0,
                    "technical_basis": "尾档止盈：保留 30% 仓位博取主升浪",
                    "fundamental_basis": "尾仓让利润奔跑",
                    "quant_signal": None,
                    "invalidation_rule": "若价格抵达后未能企稳则改为追踪止盈",
                    "priority": 52,
                })
...
"""
        ...
        # 持仓上下文存在时：追加结构化操作计划指令，要求 LLM 输出 action_plan_items
        from src.services.portfolio_context_service import build_action_plan_instruction
        prompt += build_action_plan_instruction(portfolio_context_block)
